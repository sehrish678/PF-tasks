...
// bool validCredentials(string username, string password, string Username[], string Password[], int count);
...
// //to check if the credentials entered are valid or not...
// bool validCredentials(string username, string password, string Username[], string Password[], int count)
// {
...
string products(string added)
{
   string allproducts[12] = {"PANADOL", "CALPOL", "LEFLOX", "LORIN","BRUFEN", "HYDRLIN", "ACEFYL", "GRAVINAT","SANIPLAST", "BANDAGES", "MEDICAL TAPES", "MEDICAL SCISSORS"};
   cout << added << endl;
   for (int j = 0; j < 12; j++)
   {
      cout << allproducts[j] << endl;
   }
   cout << endl;
}
...
{  
   ...
   for(int i=0;i<12;i++)
   {     
    if(allProducts[i]==deletedProduct)
    {
         continue;
    }
     cout<<allProducts[i]<<" "; 
