...
int choiceOfAdminPage(int Quantity[], int &size, string option,
                      ...
                      string password, int &feedbackiteration, string[],
                      string);
...
int choiceOfCostumer(string Password[], int count, string option, int BillCount,
                     string allMedicines[], int allProductsPrice[],
                     int medicineCount, int &size, string feedbackArray[],
                     int Quantity[], string productToBuy,
                     int &feedbackiteration, int currentIndexUser,
                     string File2);
int BuyProduct(string &productToBuy, int &quantity, string allMedicines[],
...
void LoadProducts(string allMedicines[], int allProductsPrice[], int Quantity[],
                  int &medicineCount, string File2);
void StoreProducts(string allMedicines[], int allProductsPrice[],
                   int Quantity[], int &medicineCount, string File2);
...
int choiceOfAdminPage(int Quantity[], int &size, string option,
  ...
  while (option != "9") {
    ...
    StoreProducts(allMedicines, allProductsPrice, Quantity, medicineCount,
                  File2);
  }
...
void checkFeedbacks(int &feedbackiteration, string feedbackArray[],
  ...
  setcolor(2);
  cout << "\t\t\t\t\t\t\t\t " << left << setw(20) << "Name \t\t Feedback"
       << endl;
  cout << endl;
  for (int i = 0; i < userCount; i++) {
    if (feedbackArray[i] != "")
      cout << "\t\t\t\t\t\t\t\t " << left << setw(20) << UserName[i] << "\t\t "
           << feedbackArray[i] << endl;
  }
  cout << "\t\t\t\t\t\t\t\t Press any key to continue..." << endl;
  getch();
}
...
int choiceOfCostumer(string Password[], int count, string option, int BillCount,
  ...
  while (true) {
    ...
    if (option == "1") {
      ...
      costumerPageHeader();
      ViewList(allMedicines, allProductsPrice, medicineCount, Quantity);
      BuyProduct(productToBuy, quantity, allMedicines, Quantity, size);
    } else if (option == "3") {
      system("cls");
      printheader();
      costumerPageHeader();
...
int BuyProduct(string &productToBuy, int &quantity, string allMedicines[],
  ...
  for (int i = 0; i < size; i++) {
    if (productToBuy == allMedicines[i]) {
      if (Quantity[i] == 0 ||Quantity[i] < quantity ) {
...
int BillCheck(string productToBuy, string allMedicines[],
  ...
  setcolor(2);
  if (quantity == 0) {
    cout << "\t\t\t\t\t\t\t\t There is no bill." << endl;
  }
  for (int i = 0; i < medicineCount; i++) {
    if (allMedicines[i] == productToBuy) {
      Quantity[i] = Quantity[i] - quantity;
      if(quantity<Quantity[i]||quantity==Quantity[i])
